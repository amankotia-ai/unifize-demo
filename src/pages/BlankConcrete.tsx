...
// Each layer is a recognizable collaboration tool — the abstract
// "coordination" idea, made concrete. Colours preserve the spectrum so
// the stack still reads as a balanced palette rather than eight blues.
const LAYERS: LayerSpec[] = [
  { type: "OUTLOOK", h: 0.30, color: 0xfdba74, icon: "envelope" }, // orange
  { type: "MS TEAMS", h: 0.26, color: 0xa5b4fc, icon: "meeting" }, // indigo
  { type: "SLACK", h: 0.32, color: 0x7dd3fc, icon: "chat" }, // sky
  { type: "GOOGLE DOCS", h: 0.28, color: 0xfcd34d, icon: "draft" }, // amber
  { type: "APPROVALS", h: 0.24, color: 0x6ee7b7, icon: "decision" }, // emerald
  { type: "ZOOM", h: 0.30, color: 0x5eead4, icon: "call" }, // teal
  { type: "LOOM", h: 0.28, color: 0xc4b5fd, icon: "revision" }, // violet
  { type: "@ MENTIONS", h: 0.26, color: 0xf9a8d4, icon: "mention" }, // pink
];
...
// ── Systems-of-record texture (top face of record) ─────
// Replaces the SOP document with a 2×2 grid of QMS / DMS / ERP / PLM
// product tiles, in the cobalt blue language of the reference image.
// The top of the record now reads as "the four systems of truth", which
// is what the abstract "Systems of Record" callout actually points at.
function drawSystemTile(
  ...
  // Cobalt blue gradient — mid-bright top, slightly deeper bottom
  const grad = ctx.createLinearGradient(x, y, x, y + h);
  grad.addColorStop(0, "#2f6dff"); // top highlight
  grad.addColorStop(1, "#1d4ed8"); // bottom shadow
  const r = 28;
  ...
  // 4-square brand glyph (top-left) — matches the reference image
  const logoX = x + 38;
  const logoY = y + 38;
  const cellSize = 22;
  const gapPx = 6;
  ...
  for (let r2 = 0; r2 < 2; r2++) {
    for (let c2 = 0; c2 < 2; c2++) {
      ctx.fillRect(
        logoX + c2 * (cellSize + gapPx),
        logoY + r2 * (cellSize + gapPx),
        cellSize,
        cellSize,
      );
    }
  ...
  // Tile name — large, centered (the system acronym)
  ctx.fillStyle = "rgba(255,255,255,0.98)";
  ctx.font =
    "700 168px ui-sans-serif, system-ui, -apple-system, 'Inter', sans-serif";
  ctx.textBaseline = "alphabetic";
  ctx.textAlign = "center";
  ctx.fillText(name, x + w / 2, y + h / 2 + 28);

  // Subtitle below
  ctx.fillStyle = "rgba(255,255,255,0.66)";
  ctx.font =
    "500 36px ui-sans-serif, system-ui, -apple-system, 'Inter', sans-serif";
  ctx.fillText(subtitle, x + w / 2, y + h / 2 + 92);
  ctx.textAlign = "left";

  // Bottom-right tracking dots
  ctx.fillStyle = "rgba(255,255,255,0.42)";
  ctx.font = "500 28px ui-monospace, SFMono-Regular, Menlo, monospace";
  ctx.textAlign = "right";
  ctx.fillText("· · ·", x + w - 32, y + h - 30);
  ctx.textAlign = "left";
}

function createDocumentTexture(): THREE.CanvasTexture {
  const W = 1600;
  const H = 2000; // 4 : 5 to match RECORD_W : RECORD_D
  const c = document.createElement("canvas");
  c.width = W;
  c.height = H;
  const ctx = c.getContext("2d")!;

  // Deep navy backdrop — the "systems-of-record" world
  ...
  bgGrad.addColorStop(0, "#0e1734");
  bgGrad.addColorStop(1, "#060a1c");
  ctx.fillStyle = bgGrad;
  ctx.fillRect(0, 0, W, H);

  // Subtle dot grid for atmosphere (ties in with grid floor below the tower)
  ctx.fillStyle = "rgba(140, 170, 240, 0.10)";
  const dotStep = 40;
  for (let y = 60; y < H; y += dotStep) {
    for (let x = 60; x < W; x += dotStep) {
      ctx.beginPath();
  ...
  // Top eyebrow — small monospace, far-left tracking dot
  ctx.textBaseline = "alphabetic";
  ctx.textAlign = "left";
  ctx.fillStyle = "rgba(255,255,255,0.55)";
  ctx.beginPath();
  ctx.arc(110, 140, 5, 0, Math.PI * 2);
  ctx.fill();
  ctx.font = "600 32px ui-monospace, SFMono-Regular, Menlo, monospace";
  ctx.fillStyle = "rgba(255,255,255,0.55)";
  ctx.fillText("SYSTEMS  OF  RECORD", 138, 152);

  // Headline — what this represents
  ctx.fillStyle = "rgba(255,255,255,0.92)";
  ctx.font =
    "500 60px ui-sans-serif, system-ui, -apple-system, 'Inter', sans-serif";
  ctx.fillText("The four systems that hold the truth.", 110, 220);

  // 2 × 2 tile grid
  const TILES = [
    { name: "QMS", subtitle: "Quality" },
    { name: "DMS", subtitle: "Documents" },
    { name: "ERP", subtitle: "Operations" },
    { name: "PLM", subtitle: "Product Lifecycle" },
  ];
  ...
  for (let i = 0; i < 4; i++) {
    ...
    drawSystemTile(ctx, tx, ty, tileW, tileH, TILES[i].name, TILES[i].subtitle);
  }

  // Footer metadata — anchors the page like the original SOP did
  ctx.font = "500 24px ui-monospace, SFMono-Regular, Menlo, monospace";
  ctx.fillStyle = "rgba(255,255,255,0.42)";
  ctx.letterSpacing = "1.2px";
  ctx.textAlign = "left";
  ctx.fillText("UNIFIZE  ·  SOURCE OF TRUTH", 110, H - 70);
  ctx.textAlign = "right";
  ctx.fillText("Q M S    D M S    E R P    P L M", W - 110, H - 70);
  ctx.textAlign = "left";
      ...
      >
        ...
        >
          <div className="font-mono text-[10px] uppercase tracking-[0.18em] text-white/45 md:text-[11px]">
            <span className="text-white/65">QMS · DMS · ERP · PLM</span>
            <span className="mx-3 text-white/25">·</span>
            <span className="text-white/65">Outlook · Teams · Slack · Docs · Zoom · Loom</span>
          </div>
          <div className="mt-2 font-mono text-[9px] uppercase tracking-[0.22em] text-white/25 md:text-[10px]">
            the architecture of coordination tax — made concrete
          </div>
        </div>
