...
// ── Visuals ─────────────────────────────────────────────
const EDGE = 0x000000;
// Paper hex constants are defined inside the component now (warm + cool
// palettes for the two records). Keeping the EDGE constant for parity with
// Blank.tsx.
...
// ── Layers (slightly inset so each record has a visible top-face lip) ──
// Inset reveals a paper "frame" around the SoC stack: front/back lips are
// 0.5 wide (great for showing a document title strip), left/right lips are
// 0.25 wide. This is what makes Variation A legible — both records show their
// top face, not just the topmost one.
const LAYER_W = RECORD_W - 0.5;
const LAYER_D = RECORD_D - 1.0;
...
function createDocumentTexture(): THREE.CanvasTexture {
  ...
  // Diff highlights — drawn BEFORE rows so text sits on top.
  // Each highlight marks a field whose rationale lives outside this record.
  // The pointer leaders (built later in the scene) terminate at these spots.
  const drawHighlight = (x: number, y: number, w: number, h: number) => {
    ctx.save();
    ctx.fillStyle = "rgba(245, 158, 11, 0.22)"; // amber wash
    ctx.fillRect(x, y, w, h);
    ctx.strokeStyle = "rgba(180, 100, 0, 0.55)";
    ctx.lineWidth = 1.5;
    ctx.strokeRect(x, y, w, h);
    // Tiny notch dot on the LEFT of the highlight — visual anchor
  ...
  // Rev 03 · DESCRIPTION column · "Tolerance update"
  drawHighlight(660, 905, 380, 50);
  // Rev 04 · DESCRIPTION column · "Step 4 added"
  drawHighlight(660, 1013, 320, 50);
  // Rev 04 · AUTHOR column · "K. Lee"
  drawHighlight(1180, 1013, 200, 50);
  ...
  ctx.textAlign = "right";
  ctx.fillText("CONFIDENTIAL  ·  CONTROLLED COPY", W - 110, 1968);
  ctx.textAlign = "left";
...
// ── NCR document texture (top face of bottom record) ────
// The middle of this canvas is occluded by the SoC stack above. The visible
// area is the front-lip strip (~canvas Y 0–205) and thin side strips. We bias
// the most readable content toward the front lip so the bottom record reads
// as a different document type than the top SOP.
function createNCRDocumentTexture(): THREE.CanvasTexture {
  const W = 1536;
  const H = 2048;
  const c = document.createElement("canvas");
  c.width = W;
  ...
  // Cool clinical paper background — gray/blue tint vs. SOP's warm cream.
  const bgGrad = ctx.createRadialGradient(
    W / 2,
    H / 2,
    Math.min(W, H) / 4,
  ...
  bgGrad.addColorStop(0, "#e9edf3");
  bgGrad.addColorStop(1, "#dee2eb");
  ctx.fillStyle = bgGrad;
  ctx.fillRect(0, 0, W, H);
  ...
  // FRONT-LIP STRIP — this is the section visible from iso view.
  // We design it like a binder spine label: cool gray band with key meta.
  ctx.fillStyle = "#1f2937";
  ctx.fillRect(0, 0, W, 200);

  // Severity stripe (red wedge on the left of the strip)
  ctx.fillStyle = "#dc2626";
  ctx.fillRect(0, 0, 80, 200);

  // Document number (large, mono)
  ctx.fillStyle = "#f8fafc";
  ctx.textAlign = "left";
  ctx.textBaseline = "alphabetic";
  ctx.font = "700 78px ui-monospace, SFMono-Regular, Menlo, monospace";
  ctx.fillText("NCR-2024-118", 110, 110);

  // Subtitle (smaller)
  ctx.font = "500 36px ui-sans-serif, system-ui, -apple-system, 'Inter', sans-serif";
  ctx.fillStyle = "rgba(248,250,252,0.78)";
  ctx.fillText("DEVIATION  ·  OPEN  ·  SEVERITY 2  ·  J. CHO", 110, 162);

  // Inner border for the rest of the document
  ctx.strokeStyle = "#3a3528";
  ctx.lineWidth = 3;
  ctx.strokeRect(40, 240, W - 80, H - 280);
  ctx.strokeStyle = "rgba(26,26,26,0.35)";
  ctx.lineWidth = 1;
  ctx.strokeRect(54, 254, W - 108, H - 308);

  ctx.fillStyle = "#1a1a1a";

  // Section: PRODUCT / PROCESS
  ctx.font = "600 38px ui-sans-serif, system-ui, -apple-system, 'Inter', sans-serif";
  ctx.fillStyle = "#374151";
  ctx.fillText("PRODUCT / PROCESS", 110, 360);

  ctx.font = "500 56px ui-monospace, SFMono-Regular, Menlo, monospace";
  ctx.fillStyle = "#1a1a1a";
  ctx.fillText("Batch 412B  ·  Procedure SOP-2024-072", 110, 432);

  // Section: NONCONFORMANCE
  ctx.font = "600 38px ui-sans-serif, system-ui, -apple-system, 'Inter', sans-serif";
  ctx.fillStyle = "#374151";
  ctx.fillText("NONCONFORMANCE", 110, 552);
  ctx.font = "400 36px ui-sans-serif, system-ui, -apple-system, 'Inter', sans-serif";
  ctx.fillStyle = "#1a1a1a";
  ctx.fillText("Tolerance reading 0.06 mm  ·  exceeds spec by +0.01.", 110, 612);
  ctx.fillText("Operator flagged at 04:22 prior to handoff.", 110, 660);

  // Section: ROOT CAUSE
  ctx.font = "600 38px ui-sans-serif, system-ui, -apple-system, 'Inter', sans-serif";
  ctx.fillStyle = "#374151";
  ctx.fillText("ROOT CAUSE", 110, 780);
  ctx.font = "400 36px ui-sans-serif, system-ui, -apple-system, 'Inter', sans-serif";
  ctx.fillStyle = "#1a1a1a";
  ctx.fillText("Pending  ·  awaiting QA review", 110, 840);

  // Section: DISPOSITION (checkboxes)
  ctx.font = "600 38px ui-sans-serif, system-ui, -apple-system, 'Inter', sans-serif";
  ctx.fillStyle = "#374151";
  ctx.fillText("DISPOSITION", 110, 960);
  ctx.font = "400 36px ui-sans-serif, system-ui, -apple-system, 'Inter', sans-serif";
  ctx.fillStyle = "#1a1a1a";
  const dispositions = [
    { label: "Use as-is", checked: false, y: 1024 },
    { label: "Rework", checked: false, y: 1080 },
    { label: "Reject", checked: false, y: 1136 },
    { label: "Refer to procedure update", checked: false, y: 1192 },
  ];
  for (const d of dispositions) {
    ctx.strokeStyle = "#1a1a1a";
    ctx.lineWidth = 2;
    ctx.strokeRect(110, d.y - 30, 36, 36);
    ctx.fillText(d.label, 168, d.y);
  }

  // Section: ASSIGNED
  ctx.font = "600 38px ui-sans-serif, system-ui, -apple-system, 'Inter', sans-serif";
  ctx.fillStyle = "#374151";
  ctx.fillText("ASSIGNED TO", 110, 1340);
  ctx.font = "500 56px ui-monospace, SFMono-Regular, Menlo, monospace";
  ctx.fillStyle = "#1a1a1a";
  ctx.fillText("J. CHO  ·  QA Lead", 110, 1410);
  ...
  ctx.font = "500 22px ui-monospace, SFMono-Regular, Menlo, monospace";
  ctx.fillStyle = "#7a7058";
  ctx.fillText("OPENED BY", 110, 1772);
  ...
  ctx.fillText("PAGE 1 OF 1", 110, 1968);
  ctx.textAlign = "right";
  ctx.fillText("DEVIATION  ·  CONTROLLED COPY", W - 110, 1968);
  ctx.textAlign = "left";
...
// ── Diff pill sprite (two-line pointer label) ──────────
// Used for Variation C: the pill at the end of each pointer leader carries a
// headline ("APPROVAL LIVES HERE") and a detail ("18-message email thread").
function createDiffPillSprite(
  headline: string,
  detail: string,
  accentHex: number,
): {
  ...
  ctx.quadraticCurveTo(bgX, bgY, bgX + bgR, bgY);
  ctx.closePath();
  ctx.fillStyle = "rgba(12,12,16,0.86)";
  ctx.fill();
  ctx.strokeStyle = "rgba(255,255,255,0.18)";
  ctx.lineWidth = 1.5;
  ctx.stroke();

  // Accent stripe down the left edge of the pill
  ...
  ctx.fillRect(bgX, bgY + 12, 6, bgH - 24);

  // Headline (uppercase, mono)
  ctx.fillStyle = "rgba(255,255,255,0.96)";
  ctx.font =
    "600 48px ui-monospace, SFMono-Regular, Menlo, Consolas, monospace";
  ctx.textBaseline = "middle";
  ctx.textAlign = "left";
  ctx.fillText(headline, bgX + 36, bgY + 64);

  // Detail line (regular, slightly muted)
  ctx.fillStyle = "rgba(255,255,255,0.72)";
  ctx.font =
    "400 38px ui-sans-serif, system-ui, -apple-system, 'Inter', sans-serif";
  ctx.fillText(detail, bgX + 36, bgY + 132);
  ...
  const sprite = new THREE.Sprite(material);
  // World-space size: slightly bigger than arch labels so the two-line text
  // stays readable.
...
// ── Diff pointer (anchor → leader polyline → pill label) ──
// Variation C: a leader connects a highlighted field on the SOP top face to a
// pill label outside the tower, passing through the side of the SoC layer
// that's actually holding the rationale/approval/exception.
function createDiffPointer(opts: {
  anchor: THREE.Vector3;       // anchor on top face of top record (worldspace)
  layerEntryY: number;          // y of the target SoC layer's vertical center
  pillX: number;                // x of pill label (typically negative, off to left)
  pillY: number;                // y of pill label
  pillZ: number;                // z of pill label (typically 0)
  headline: string;
  detail: string;
  accentHex: number;
}): {
  ...
  const { anchor, layerEntryY, pillX, pillY, pillZ, headline, detail, accentHex } = opts;
  const group = new THREE.Group();

  // Shared line material — leader and any sub-elements animate together
  const lineMaterial = new THREE.LineBasicMaterial({
    color: 0xffffff,
    transparent: true,
    opacity: 0,
    depthWrite: false,
    depthTest: false, // draw on top of everything for clarity
  });

  // Leader polyline:
  //   A: anchor on document
  //   B: small riser above the document (clears the surface)
  //   C: bend point above the side of the target SoC layer
  //   D: entry point at the side of the target SoC layer (drop into layer)
  //   E: pill label position (further out)
  const A = anchor.clone();
  const B = new THREE.Vector3(anchor.x, anchor.y + 0.18, anchor.z);
  const C = new THREE.Vector3(-LAYER_W / 2 - 0.1, anchor.y + 0.18, 0);
  const D = new THREE.Vector3(-LAYER_W / 2 - 0.1, layerEntryY, 0);
  const E = new THREE.Vector3(pillX + 1.5, pillY, pillZ);

  const leaderGeo = new THREE.BufferGeometry().setFromPoints([A, B, C, D, E]);
  const leaderLine = new THREE.Line(leaderGeo, lineMaterial);
  leaderLine.renderOrder = 5;
  group.add(leaderLine);

  // Tiny dot at the anchor (visually anchors the leader to the document)
  ...
  // Tiny chip at the entry point (where the leader enters the SoC layer)
  const chipGeo = new THREE.SphereGeometry(0.05, 12, 12);
  ...
  chip.position.copy(D);
  chip.renderOrder = 5;
  group.add(chip);

  // Pill label
  const pill = createDiffPillSprite(headline, detail, accentHex);
  pill.sprite.position.set(pillX, pillY, pillZ);
  group.add(pill.sprite);

  // Tie chip and dot opacity to a single material handle for animation —
  // we'll reuse the lineMaterial for them via a shared opacity update.
  const pulseChip = () => {
    dotMat.opacity = lineMaterial.opacity;
    chipMat.opacity = lineMaterial.opacity;
  };
  // Hijack lineMaterial.onBeforeRender (per-frame) to sync the small chips —
  // simpler than tracking two extra materials in animate().
  // Three.js LineBasicMaterial has no per-frame hook by default, but the
  // material's userData can carry the sync function which the animate loop
  // calls. We attach it here.
  (lineMaterial.userData as { syncChips?: () => void }).syncChips = pulseChip;

  return {
    group,
    spriteMaterial: pill.material,
    lineMaterial,
    texture: pill.texture,
    geometries: [leaderGeo, dotGeo, chipGeo],
  };
}
...
export default function BlankAC() {
  ...
  useEffect(() => {
    ...
    const sopTexture = createDocumentTexture();
    const ncrTexture = createNCRDocumentTexture();
    disposables.push(sopTexture, ncrTexture);
    ...
    // Two paper palettes — warm cream (process record) and cool clinical
    // (quality record). Different at the eye-level before any label reads.
    const WARM_SHEET_COLORS = [
      0xede5c2, 0xf0e8c6, 0xece4c0, 0xefe7c4, 0xf2eaca,
      0xede5c1, 0xf0e8c8, 0xece4c1, 0xf1e9c8, 0xefe7c4,
      0xf2eacb, 0xf4eed8,
    ];
    const COOL_SHEET_COLORS = [
      0xc9cfdb, 0xced4df, 0xc6ccd8, 0xd0d6e1, 0xd5dbe6,
      0xc8cedb, 0xcfd5e0, 0xc7cdd9, 0xd2d8e3, 0xcfd5e0,
      0xd6dce6, 0xdee4ee,
    ];
    const WARM_TOP_HEX = 0xf4eed8;
    const COOL_TOP_HEX = 0xeef2f8;
    const WARM_EDGE = 0x6b5e3a;
    const COOL_EDGE = 0x3a4252;

    // Helper: build a paper-sheet stack at a given baseY with a chosen palette
    const buildPaperStack = (opts: {
      baseY: number;
      docTexture: THREE.Texture | null;
      rngSeed: number;
      palette: number[];
      topHex: number;
      edgeColor: number;
    }) => {
      const { baseY, docTexture, rngSeed, palette, topHex, edgeColor } = opts;
      ...
      for (let i = 0; i < NUM_PAPER_SHEETS; i++) {
        ...
        const sheetColor = palette[i] ?? palette[palette.length - 1];
        const useDoc = isTopOfStack && docTexture !== null;
        ...
        const sheet = createTexturedSlab({
          ...
          topColor: useDoc ? topHex : sheetColor,
    ...
    // 1. Bottom record stack — Quality Record (NCR), cool clinical paper.
    //    Its top face is partly occluded by the SoC layers above, but the
    //    front-lip strip carries the title where it's readable from iso view.
    buildPaperStack({
      baseY: 0,
      docTexture: ncrTexture,
      rngSeed: 31,
      palette: COOL_SHEET_COLORS,
      topHex: COOL_TOP_HEX,
      edgeColor: COOL_EDGE,
    });
    ...
    // 3. Top record stack — Process Record (SOP), warm cream paper.
    //    Its top face has the SOP document texture, complete with the diff
    //    highlight markers that the pointer leaders (below) connect to.
    buildPaperStack({
      baseY: yCursor,
      docTexture: sopTexture,
      rngSeed: 73,
      palette: WARM_SHEET_COLORS,
      topHex: WARM_TOP_HEX,
      edgeColor: WARM_EDGE,
    });
    ...
    // ── Variation C: diff pointers ─────────────────────────
    // Three highlighted fields on the SOP top face point into the side of
    // specific SoC layers, with pill labels naming what each layer holds.
    //
    // SOP texture is 1536×2048. Top face of the top stack is at y=3.24 and
    // spans world (-2..+2, _, +2.5..-2.5). The notch dot for each highlight
    // is drawn on the texture at (x-14, y+h/2) in canvas coords; the world
    // mapping is:
    //   worldX = (canvasX/1536)*4 - 2
    //   worldZ = (1 - canvasY/2048)*5 - 2.5
    //
    // Anchors come from the highlight rects in createDocumentTexture():
    //   Tolerance update (Rev 03): canvas notch (646, 930)
    //   Step 4 added    (Rev 04): canvas notch (646, 1038)
    //   K. Lee approver (Rev 04): canvas notch (1166, 1038)
    const sopTopY = topRecordTop;
    const canvasToWorld = (cx: number, cy: number): THREE.Vector3 =>
      new THREE.Vector3(
        (cx / 1536) * RECORD_W - RECORD_W / 2,
        sopTopY,
        (1 - cy / 2048) * RECORD_D - RECORD_D / 2,
      );

    // Layer y-centers (from LAYERS array, cumulative from y=0.5)
    const layerEmail = 0.5 + 0.30 / 2;            // 0.65 — EMAIL THREAD
    const layerMeeting = 0.5 + 0.30 + 0.26 / 2;   // 0.93 — MEETING
    const layerChat = 0.5 + 0.30 + 0.26 + 0.32 / 2; // 1.22 — CHAT THREAD

    const diffPointers = [
      createDiffPointer({
        anchor: canvasToWorld(1166, 1038),
        layerEntryY: layerEmail,
        pillX: -5.4,
        pillY: layerEmail,
        pillZ: 0,
        headline: "APPROVAL LIVES HERE",
        detail: "18-message email thread",
        accentHex: 0xfdba74, // matches EMAIL THREAD layer color
      }),
      createDiffPointer({
        anchor: canvasToWorld(646, 1038),
        layerEntryY: layerMeeting,
        pillX: -5.4,
        pillY: layerMeeting + 0.55,
        pillZ: 0,
        headline: "RATIONALE LIVES HERE",
        detail: "47-min meeting · no notes",
        accentHex: 0xa5b4fc, // matches MEETING layer color
      }),
      createDiffPointer({
        anchor: canvasToWorld(646, 930),
        layerEntryY: layerChat,
        pillX: -5.4,
        pillY: layerChat + 1.1,
        pillZ: 0,
        headline: "EXCEPTION LIVES HERE",
        detail: "decided in chat · lost",
        accentHex: 0x7dd3fc, // matches CHAT THREAD layer color
      }),
    ];

    for (const p of diffPointers) {
      scene.add(p.group);
      disposables.push(p.texture, p.spriteMaterial, p.lineMaterial);
      for (const g of p.geometries) disposables.push(g);
    }
    ...
    const animate = () => {
      ...
      // Architectural callouts (PROCESS RECORD / QUALITY RECORD) fade in
      // just before the camera tween completes — the vocabulary "lands" as
      ...
      // Diff pointers fade in after the architectural labels — staggered
      // 0.25s apart so each pointer registers in turn.
      const POINTER_BASE_START = 2.5;
      const POINTER_DURATION = 0.6;
      const POINTER_STAGGER = 0.25;
      diffPointers.forEach((p, idx) => {
        const start = POINTER_BASE_START + idx * POINTER_STAGGER;
        const pT = Math.max(
          0,
          Math.min((t - start) / POINTER_DURATION, 1),
        );
        const eased = 1 - Math.pow(1 - pT, 3);
        p.spriteMaterial.opacity = 1.0 * eased;
        p.lineMaterial.opacity = 0.65 * eased;
        // Sync the chip+dot meshes (Three.js MeshBasicMaterial doesn't share
        // opacity with the line material automatically — see createDiffPointer).
        const userData = p.lineMaterial.userData as {
          syncChips?: () => void;
        };
        if (userData.syncChips) userData.syncChips();
      });
      ...
      >
        ...
        >
          ...
          <div className="mt-1 font-serif text-base font-medium text-white/85 md:text-lg">
            Two records. Cost mapped.
          </div>
        </div>

        {/* Vocabulary caption — bottom-center, fades in after pointers land */}
        <div
          className="pointer-events-none absolute bottom-8 left-1/2 z-10 -translate-x-1/2 text-center md:bottom-10"
          style={{ animation: "blankCaptionFade 1s ease-out 3.7s both" }}
        >
          <div className="font-mono text-[10px] uppercase tracking-[0.18em] text-white/45 md:text-[11px]">
            <span className="text-white/65">process record</span>
            <span className="mx-3 text-white/25">·</span>
            <span className="text-white/65">quality record</span>
            <span className="mx-3 text-white/25">·</span>
          ...
          <div className="mt-2 font-mono text-[9px] uppercase tracking-[0.22em] text-white/25 md:text-[10px]">
            approval, rationale, exception. all in flight.
          </div>
        </div>
